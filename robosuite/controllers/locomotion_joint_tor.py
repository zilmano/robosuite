from robosuite.controllers.base_locomotion_controller import LocomotionController
import numpy as np


class LocomotionJointTorqueController(LocomotionController):
    """
    ...
    Args:
        ...
        input_max (float or list of float): Maximum above which an inputted action will be clipped. Can be either be
            a scalar (same value foFFr all action dimensions), or a list (specific values for each dimension). If the
