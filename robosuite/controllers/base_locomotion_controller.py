...
class LocomotionController(object, metaclass=abc.ABCMeta):
    """
    ...
    def __init__(self,
        ...
        self.chassis_ori_mat = None
        self.chassis_pos_vel = None
        self.chassis_ori_vel = None
        self.joint_pos = None
        self.joint_vel = None
        ...
        self.initial_chassis_pos = self.chassis_pos
        self.initial_chassis_ori_mat = self.chassis_ori_mat
    ...
    def update(self, force=False):
        ...
        if self.new_update or force:
            ...
            self.chassis_pos = np.array(self.sim.data.body_xpos[self.sim.model.body_name2id(self.chassis_name)])
            self.chassis_ori_mat = np.array(self.sim.data.body_xmat[self.sim.model.body_name2id(self.chassis_name)].reshape([3, 3]))
            self.chassis_pos_vel = np.array(self.sim.data.body_xvelp[self.sim.model.body_name2id(self.chassis_name)])
            self.chassis_ori_vel = np.array(self.sim.data.body_xvelr[self.sim.model.body_name2id(self.chassis_name)])

            self.joint_pos = np.array(self.sim.data.qpos[self.qpos_index])
            self.joint_vel = np.array(self.sim.data.qvel[self.qvel_index])

            self.J_pos = np.array(self.sim.data.get_body_jacp(self.chassis_name).reshape((3, -1))[:, self.qvel_index])
            self.J_ori = np.array(self.sim.data.get_body_jacr(self.chassis_name).reshape((3, -1))[:, self.qvel_index])
